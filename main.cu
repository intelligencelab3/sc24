...
string base_dir = "/data4/yul23028/MergePath-SpMM/ICCAD-Accel-GCN/graphs/";
...
double check_err(float *out, float *out_ref, int len, bool &has_err)
{
    ...
    for (int i = 0; i < len; i++)
    {
        double err = abs(out[i] - out_ref[i]);//check the sum of all absolute value of differences in a comparable range and check very single absolute value

        // cout<<" out: "<<out[i]<<" out_ref: "<<out_ref[i]<<endl;
        ...
        {
            has_err = 1;
            cout << "err at " <<( i /32) << endl;
            cout << "how much err: " << err << endl;
        }
    }
...
void test_graph(string graph, int spec_dim, bool checking, bool timing)
{
    ...
    // cout<<"v_num: "<<v_num<<" "<<"e_num: "<<e_num<<endl;

    int v_num_old = cuda_read_array(&cu_indptr, base_dir + graph + ".graph.ptrdump");
    int e_num_old = cuda_read_array(&cu_indices, base_dir + graph + ".graph.edgedump");
    // cout<<"v_num_old: "<<v_num_old<<" "<<"e_num_old: "<<e_num_old<<endl;
...
// newly added
    ...
    // printf("Filling kernels with data!\n");
...
#define TIMING
    ...
    for (int dim = dim_min; dim <= dim_max; dim += interval)
    {
        ...
        if(checking){
            ...
            // accel.do_test(true, dim);
            // gnna.do_test(true, dim);
